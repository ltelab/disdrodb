...
def df_sanitizer_fun(df, lazy=False):
    ...
    df[["rainfall_rate_32bit", "TO_SPLIT"]] = df["TO_SPLIT"].str.split(
        " ", n=1, expand=True
    )

    df["TO_SPLIT"] = df["TO_SPLIT"].str.strip()

    df[["rainfall_accumulated_32bit", "TO_SPLIT"]] = df["TO_SPLIT"].str.split(
        " ", n=1, expand=True
    )

    df["TO_SPLIT"] = df["TO_SPLIT"].str.strip()

    df[["reflectivity_32bit", "TO_SPLIT"]] = df["TO_SPLIT"].str.split(
        " ", n=1, expand=True
    )

    df["TO_SPLIT"] = df["TO_SPLIT"].str.strip()

    df[["number_particles", "TO_SPLIT"]] = df["TO_SPLIT"].str.split(
        " ", n=1, expand=True
    )
    ...
    df[["raw_drop_concentration", "TO_SPLIT"]] = df["TO_SPLIT"].str.split(
        " ", n=1, expand=True
    )

    df[["raw_drop_average_velocity", "raw_drop_number"]] = df["TO_SPLIT"].str.split(
        " ", n=1, expand=True
    )
