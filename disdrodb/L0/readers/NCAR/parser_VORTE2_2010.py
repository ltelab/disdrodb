...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 14 09:59:10 2022
...
def main(raw_dir,
    ...
    def df_sanitizer_fun(df, lazy=False):
        ...
        # Delete invalid columns by check_L0A
        col_to_drop = ['rainfall_rate_16_bit_1200', 'rainfall_rate_16_bit_30', 'sample_interval', 93,94,95,96,97]
        ...
        # Cast time
        df["time"] = dd.to_datetime(df["sensor_date"] + '-' + df["sensor_time"], format="%d.%m.%Y-%H:%M:%S")
        ...
        if set(['weather_code_metar_4678','weather_code_nws']).issubset(df.columns):
            df['weather_code_metar_4678'] = df['weather_code_metar_4678'].str.strip()
            df['weather_code_nws'] = df['weather_code_nws'].str.strip()

        # Cast values
        col_list = ['mor_visibility', 'number_particles', 'sensor_temperature',]
        for c in col_list:
            df[c] = df[c].str.split('.').str[0].astype(float)

        # Format raw_drop_number (not all station have the same format)
        if df['raw_drop_number'].str.contains('0.00000').any():
            r = df['raw_drop_number'].str.split(';')
            for index, value in r.items():
                ...
                for i, v in enumerate(value):
                    if v == 'NaN':
                        ...
                        temp_value += '%03d' % int(v.split('.')[0]) + ','
                r[index] = temp_value
            df['raw_drop_number'] = r
            
        # Replace ';' with ',' for raw_drop_concentration, raw_drop_average_velocity and raw_drop_number
        if 'raw_drop_concentration' in df:
            df['raw_drop_concentration'] = df['raw_drop_concentration'].str.replace(';',',')
        if 'raw_drop_average_velocity' in df:
            df['raw_drop_average_velocity'] = df['raw_drop_average_velocity'].str.replace(';',',')
        if 'raw_drop_number' in df:
            df['raw_drop_number'] = df['raw_drop_number'].str.replace(';',',')
            
        # Replace NaN with real nan
