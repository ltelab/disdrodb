...
def reader(
    ...
    df_spectrum["datetime"] = pd.to_datetime(df_spectrum["date"] + " " + df_spectrum["time"], format="%d/%m/%Y %H:%M:%S")
    ...
    df_raw_drop_number = (df_spectrum
        .groupby("group_time", as_index=False)
    ...
    # Define datetime 
    df_data["datetime"] = pd.to_datetime(df_data["date1"] + " " + df_data["time1"], 
                                         format="%d/%m/%Y %H:%M:%S")
