...
def reader(
    ...
    def read_txt_file(file, filename, logger):
        ...
        try:
            ...
            log_warning(
                logger, msg=f"Corrupted raw_drop_number or peak_to_pedestal_hist field in {filename}", verbose=False
            )
            return None
        # Parse fields
        data = {
            ...
            "peak_to_pedestal_hist": (
                peak_to_pedestal_hist  # noqa Peak to pedestal ratio distribution histogram (message field 48) noqa
            ),
        }
