...
########################## Calculation of nth order moment from DSD ##############################
moment_fromDSD <- function(DSD,order,prefactor,input.matrix=FALSE){
	## Inputs:
	##   'DSD': A matrix with Volumic DSD data. Each row corresponds to a measurements, column are the diameter classes. DSD has to be given in [m-3.mm-1]
        ##   'order': order of the moment to calculate
        ##   'prefactor': prefactor of the moment (e.g., 6e-4*pi*3,778 for R (D in mm, v in m/s) from Atlas, 1977)
	## Optional arguments:
	##   'input.matrix': logical, is input DSD a matrix ? by default= FALSE (i.e. vector of length 32).
	## Outputs:
	##   'M': A vector of M values in

	# Written by Alexis Berne, April 2010
  ...
  for (meas in 1:n_meas){
    if(input.matrix == TRUE){dsd <- DSD[meas,]}else{dsd <- DSD}
    M[meas] <- prefactor*sum(dsd*class_size^order*class_spread,na.rm=FALSE)
  }
