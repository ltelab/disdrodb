...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  3 12:11:55 2022
...
# First n files for every device
import random
random_files = []
...
for d in list_stations_id:
    glob_pattern = os.path.join("data", d, ext)
    device_path = os.path.join(raw_dir, glob_pattern)
    file_list = sorted(glob.glob(device_path, recursive = True))
    random.shuffle(file_list)
    random_files += file_list[:n]
...
reader_kwargs['na_values'] = ['na', '', 'error', 'NA', '-.-', 'CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC']
